...
It also requires data prepared, currently using a simulated dataset.
...
def do_setup():
    ...
    df = pd.read_csv(f"{path_to_root}/data/simulated/downsampled.csv").drop(columns = ['Unnamed: 0'])
    ...
    # Convert the DataFrame into a list of Document objects that the index can understand
    documents = [Document(text=row['body'],
                        metadata={'title': row['title']}) for index, row in df.iterrows()]  # NOTE: could try to include AI_Label for sentiment/topic in the metadata here
    ...
    storage_context = StorageContext.from_defaults(persist_dir=f"{path_to_vector_store}/simulated_dataset_nodes_index") 
